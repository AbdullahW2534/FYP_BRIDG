...
function ViewOrders() {
    ...
    useEffect(() => {
        ...
        axios.get(`${import.meta.env.VITE_APP_BACKEND_URL}/order/getOrders`)
            .then(res => {
                setOrders(res.data);
            })
                        ...
                        {ordersData.map((order, index) => (
                            <div key={order._id} className='w-full flex flex-col  shadow-sm shadow-gray-300  pb-4'>
                                <h2 className='flex justify-center items-center px-2 w-full border-b border-gray-200 font-bold bg-purple-300'>
                                    {/* <Unicons.UilCheckCircle width={25} height={25} className="text-green-500 mr-1 cursor-pointer hover:bg-green-500 hover:text-white rounded-lg" onClick={() => handleApproval(order, "approved")} />
                                    <Unicons.UilTimesCircle width={25} height={25} className="text-red-500 hover:bg-red-500 hover:text-white rounded-lg cursor-pointer" onClick={() => handleApproval(order, "reject")} /> */}
                                    TRXREF : {order.trxref}
                                </h2>
